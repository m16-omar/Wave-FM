...
// Fill missing days with the same entries for seamless interactivity
SCHEDULE_DAYS.forEach((day) => {
  if (!SCHEDULE_DATA[day].length) {
    SCHEDULE_DATA[day] = SCHEDULE_DATA.friday;
  }
});

export const WeeklySchedule: React.FC = () => {
  const [activeDay, setActiveDay] = useState<DayOfWeek>('friday');
  const { playLiveStream } = useAudio();

  const entries = SCHEDULE_DATA[activeDay] || SCHEDULE_DATA.friday;

  return (
    <section className="w-full py-8 sm:py-12 select-none">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        {/* Dark Container Card from Screenshot 3 */}
        <div className="bg-[#141416] rounded-[28px] sm:rounded-[32px] overflow-hidden border border-white/10 shadow-2xl">
          ...
          <div className="divide-y divide-white/5">
            {entries.map((item) => (
              <div
                key={item.id}
                className="p-3 sm:p-4 flex items-center justify-between gap-3 sm:gap-4 hover:bg-white/[0.03] transition-colors group"
              >
                ...
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  ...
                  <div className="relative w-20 sm:w-28 h-12 sm:h-14 rounded-lg overflow-hidden bg-neutral-900 shrink-0 border border-white/10 flex items-center">
                    ...
                    <div className="absolute right-2 w-8 h-8 rounded-full overflow-hidden border border-white/20">
                      <img
                        src={item.hostAvatar}
                        alt={item.hostName}
